...
fn query_ex(ctx: &mut CommandContext, decide: bool, op: i64) -> i64 {
    ...
    match op {
        o if o == ctx.ids.key_op_on_down as i64 => {
            bool_i64(keys.iter().any(|&k| key_enabled(ctx, k) && ctx.input.vk_down_stock(k)))
        }
        o if o == ctx.ids.key_op_on_up as i64 => {
            bool_i64(keys.iter().any(|&k| key_enabled(ctx, k) && ctx.input.vk_up_stock(k)))
        }
        o if o == ctx.ids.key_op_on_down_up as i64 => {
            bool_i64(keys.iter().any(|&k| key_enabled(ctx, k) && ctx.input.vk_down_up_stock(k)))
        }
        o if o == ctx.ids.key_op_is_down as i64 => {
            bool_i64(keys.iter().any(|&k| key_enabled(ctx, k) && ctx.input.vk_is_down(k)))
        }
        o if o == ctx.ids.key_op_is_up as i64 => {
            bool_i64(keys.iter().all(|&k| !key_enabled(ctx, k) || !ctx.input.vk_is_down(k)))
        }
        _ => 0,
    }
}
...
fn query_vk(ctx: &mut CommandContext, vk: u8, op: i64) -> i64 {
    if !key_enabled(ctx, vk) {
        return if op == ctx.ids.key_op_is_up as i64 { 1 } else { 0 };
    }
    match op {
...
fn dir_mask_filtered(ctx: &CommandContext) -> i64 {
    let mut m = 0;
    if key_enabled(ctx, 0x25) && ctx.input.vk_is_down(0x25) {
        m |= 1;
    }
    if key_enabled(ctx, 0x27) && ctx.input.vk_is_down(0x27) {
        m |= 2;
    }
    if key_enabled(ctx, 0x26) && ctx.input.vk_is_down(0x26) {
        m |= 4;
    }
    if key_enabled(ctx, 0x28) && ctx.input.vk_is_down(0x28) {
        m |= 8;
    }
    m
}
