...
/// SYSTEM global form.
///
/// This tends to cover process/platform state, save/load UI, config switches, etc.
/// For bring-up, we implement a conservative property-bag model (no guessing).
pub fn dispatch(ctx: &mut CommandContext, form_id: u32, args: &[Value]) -> Result<bool> {
    let (op, params, rhs_assign) = if let Some((pos, chain)) = parse_element_chain(form_id, args) {
        ...
        let Some(op_i64) = args.get(0).and_then(|v| v.as_i64()) else {
            ctx.unknown.record_unimplemented("SYSTEM/invalid-op");
            ctx.push(Value::Int(0));
            return Ok(true);
        };
        let params = &args[1..];
    ...
    if v == 0 {
        ctx.unknown
            .record_unimplemented(&format!("SYSTEM/get op={op}"));
    }
