...
extern "C" {
    ...
    fn tfh_videoinfo(h: *mut TfHandle, width: *mut c_int, height: *mut c_int, fps: *mut c_double, fmt: *mut c_int);
...
impl TheoraFile {
    pub fn open_from_memory(data: Vec<u8>) -> Result<Self> {
        ...
        let handle = unsafe { tfh_open_callbacks(datasource.as_mut() as *mut _ as *mut c_void, io) };
        if handle.is_null() {
        ...
        Ok(Self {
            handle,
            datasource,
            info: VideoInfo { width, height, fps, fmt },
        })
    }
