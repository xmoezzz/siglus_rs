...
if ! "${ADB_BIN}" devices | awk 'NR>1 && $1 ~ /^emulator-[0-9]+$/ && $2=="device"{found=1} END{exit(found?0:1)}'; then
  ...
  nohup "${EMULATOR_BIN}" -avd "${AVD_NAME}" -netdelay none -netspeed full >/tmp/siglus_emulator.log 2>&1 &
fi
...
if [[ -z "${SERIAL}" ]]; then
  ...
  echo "Hint: check /tmp/siglus_emulator.log" >&2
  exit 1
fi
...
if [[ "${SHOW_LOGCAT}" == "1" ]]; then
  ...
  "${ADB_BIN}" -s "${SERIAL}" logcat -v brief | grep -E "(${PKG}|siglus|Siglus)" || true
fi
