...
if [[ ! -d "$VENDOR" ]]; then
  echo "[ios] Missing $VENDOR, building Siglus.xcframework..."
  "$SCRIPT_DIR/build_ios_xcframework.sh"
fi

[[ -d "$VENDOR" ]] || { echo "ERROR: Siglus.xcframework still missing after build: $VENDOR" >&2; exit 1; }
...
APP_PATH="$DERIVED/Build/Products/$CONFIG-iphoneos/Siglus.app"
if [[ ! -d "$APP_PATH" ]]; then
...
IPA_OUT="$DIST_DIR/SiglusLauncher.ipa"
rm -f "$IPA_OUT"
